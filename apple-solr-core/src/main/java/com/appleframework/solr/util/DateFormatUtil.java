 ...
 */
package com.appleframework.solr.util;
...
public final class DateFormatUtil {
	 ...
	 */
	public final static ThreadLocalDateFormat FORMAT_THREAD_LOCAL = new ThreadLocalDateFormat(new ISO8601CanonicalDateFormat());
	...
	/**
	 * Thread safe method that can be used to parse a Date without the trailing
	 * 'Z'
	 */
		...
		public Date parse(String i, ParsePosition p) {
			...
			if (null != d && -1 == p.getErrorIndex() && milliIndex + 1 < i.length() && '.' == i.charAt(milliIndex)) {
				...
				if (-1 == p.getErrorIndex()) {
					...
					d = new Date(
							d.getTime() + (long) (millis.doubleValue() * Math.pow(10, (3 - endIndex + milliIndex))));
				}
			}
			return d;
		}
